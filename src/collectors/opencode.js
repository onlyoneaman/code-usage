...
export function collectOpencode(options = {}) {
  ...
  for (const msg of messages) {
    ...
    // Project aggregation
    const cwd = data.path?.cwd ? data.path.cwd : sess ? sess.directory : "";
    if (cwd) {
  ...
  const projectsArr = Object.entries(projAgg)
    .map(([path, p]) => {
...
function tryStorage(storageDir, dbPath) {
  ...
  let files;
  try {
    files = readdirSync(sessionDiffDir).filter((f) => f.endsWith(".json"));
  } catch {
    return null;
  }
